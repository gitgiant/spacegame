...
Have the intial screen on the menu be artwork depicting the place.  Have each menu in the port have a comms pop up and welcome message from the shopowner that is placed only once.  
...
chang factions to just color name - place holder for now
...
shockwave/pushpack/defensive tools
ship abilities/ cooldowns.
...
have minimap circular and simulate radar + jamming.  IR vision can see cloaking.  add cloaking.

comandeer derelict vessels.

ship captian personalitys, have them cuss, be nice, salty, tired sounding, angry, authoritarian, rebel.

when the comms menu is open allow the player to steer the ship with wasd and arrow keysa

smuggling, scanning, npc scanner ships, black markets at catinas, 
...
demand tribute from planets, take over planets, have player run own faction.  faction war, planet faction fliipping
...
when ships hyperspace away, dont destroy them.
...
when player creates new game have them create a new main character, with name, sex, and avatar. For now generate 1 male and 1 female avatar for each factio$

when a comms message is being displayed, have the message appear letter by letter, and when a letter appears have the animal crossing style animalese effec$

when the player lands somewhere, have a comms message from space traffic control welcoming the player.  Have the intial screen on the menu be artwork depic$


mobile controls mode.  key mapping config.
...
comandeer derelict ships, have a chance to be damaged and require repairs, but can be a cheap way to get a new ship.

caravans

dave the diver style resteraunt play

steal everything from rimworld and rimworld mods

comms on escort ship order to hold position, attack, defend
lock in formationwwwwwwwwwwwwwwwwwwd
...
dumb missle (rocket) vs IR missle vs radar missle vs radiation missle vs optical vs laser targeted (player steered) missle
...
make the ship grid a 9x9 grid.  resize the ship grid to fit in display.  permenantly place the ship class core in the center of the 9x9 grid. Desing each ship have a starting module placement. design all ships moduile placement to follow these design principles:  follow a symetric design, the core is in the center, all other modules are attached from the core.  engine modules have at least 2 backwards pointing and one forward.  weapons point forward.  larger ships have more hull cores.
l

add abilities to the game:
...
when a player creates a character have them pick a staring spell.  have cooldowns for each abiltiy.  abilties are tied to characters, therefore all ships can have abilties.  have the NPC ships use abilities if they have them.
...
Chaff: emit a cloud of chaff behind the player that expands into an area effect, breaking all locks and any missles that hit the chaff explode.

Quantume brake: instantly stop the players ship to 0 velocity.

decroratiive modules
...
have small amount of faction kill rep gain in all space.
 ...
 meelee ships, ramming? 

 allow ships that spawn in from events from different factions should be able ot attack each other

drastically decrease time to kill
make missiles have a cooldown, more missles erquire more missile modules
...
remove reverse abillity if there is no forward facing thruster.
...
for parameter tuning for balance, make all parameters have deafult but allow player to double click and edit them if debug mode is on.34
...
multiplater scenarios
deathmatch
team deathmatch
non-even team scenarios
coop
or just mmo style "safe zones" and "pvp zones" 

Targeting pod
radar module - simulate radar, make radar ping an ability
RWR module - if hit by radar ping then play a chrip sound and show a general direction of the ping
stealth ships - hull that absorbs radar, slower quieter enginers
...
NPC satelites spawns - loot, may or may not be defensless
...
have missles launch .5 seconds after detatching from ship. have missiles emit a high pitched hissing noise when flying.  play explosion sound when missile explodes. have missles run out of fuel and start difting to a stop.  if a missle flys near a chaff cloud have it lose lock

chaff ball launching sound

pan all sounds happening around the players ship in their proper stereo channel.  

play weapon sound based on projectile type: laser, kenetic, rail, etc\

meelee ship - no booster but spprint, have move directionally and not consesrve momentum not by newtonian physics

have ship names in different languages (german, etc)

solar wind

npc ships that have chaff equipped will use it if they have a missle locked onto them and it is about to hit them.
...
larger ships have "+" shaped core
 ...
 mousewheel zoom out during spaceflight zooms into a new intermediate map state called hex map which is hex grid of the sector.   in this view, draw any objects that are visible in the minimap as the same icon and color (basically make it a blown up minimap). make the current sector size about the size of one hex.  move planets, spaceports, and stars in the system into different hexe to make systems larger.  fleets that warp in that are hostile to the sector must warp in an empty hex away from the core of the system.  
 ...
 missles trigger from countdown and give autofire option.  have multiple missle lock.

 ship flight close to planet, have atmosphere cause drag.

make each ship have focused, unique playstyle
shuttle: carry cargo, run away, turret shoots pursuers, chaff for missles.  cloak?
