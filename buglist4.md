...
make each ship unique ship name and captain name.  have ship reflect personality of the captain.
...
in the target screen, list the ship name first, then faction (in their faction color), then their hostility status.
...
title screen hyperspace animation
...
for installation of modules have the ship builder at port.  inside the shipbuilder show a grid representing the ship.  add cockpit module which all ships require to be equipped on the ship to make it flyable. allow the player to drag and drop module tiles from their module inventory onto the ship grid.  only allow the player to install as many modules as slots are available on the ship.
...
allow rotation of modules.  when module is selected in ship builder, q and e rotate it.
place splitter module in front of directed weapon module > splitter
silencer
have irregular shaped modules.  make a puzzle / tetris out of it.
...
status effects, cooldown spells, abilities: grappling hook, tractor beam, super shield, invuln, pushback, emp (disable own shields), lifesteal, swap, jammer, hacker, damage, silence, slow, freeze, fire, IR vision can see cloaking
...
have events spawn: small/medium/large fleet arriving, multiple fleets fighting, merchant fleet arriving, merchant fleet being pursued by pirates, etc.  when events are triggered write down info in the log.
